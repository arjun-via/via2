...
class DockerExecutor:
    ...
    def start(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                timeout=300,  # 5 min for image pull
                check=True
            )
    ...
    def apply_patch(self, patch: str) -> ExecutionResult:
        ...
        # If that fails, try with --unidiff-zero (handles empty @@ lines)
        result2 = self.execute("git apply --unidiff-zero /tmp/opus_patch.diff")
        if result2.success:
            return result2

        # If still fails, try patch command which is more lenient
        result3 = self.execute("patch -p1 < /tmp/opus_patch.diff")
        if result3.success:
            return result3
...
def get_swebench_image(instance_id: str) -> str:
    ...
    The SWE-bench image naming convention is:
        swebench/sweb.eval.x86_64.{repo}_{version}_{repo}-{issue_number}:latest

    Example:
        django__django-13023 -> swebench/sweb.eval.x86_64.django_1776_django-13023:latest
        astropy__astropy-12907 -> swebench/sweb.eval.x86_64.astropy_1776_astropy-12907:latest
    """
    # Parse instance_id: "django__django-13023" -> repo="django", issue="django-13023"
    parts = instance_id.split("__")
    if len(parts) == 2:
        repo = parts[0]  # "django"
        issue = parts[1]  # "django-13023"

        # Default version for SWE-bench Verified
        version = "1776"
